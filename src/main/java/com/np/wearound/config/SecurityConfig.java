...
public class SecurityConfig {
   ...
   public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
      ...
      .and()
      .addFilterBefore(new JwtAuthFilter(userAuthProvider), BasicAuthenticationFilter.class) // Spring security의 인증필터 앞에 JWT 필터 추가
...
//            .antMatchers(HttpMethod.POST, "/login", "/register","/logout").permitAll() // 인증이 필요하지 않은 유일한 엔드포인트,리액트의 url과 일치시켜야 함
            .antMatchers("/**").permitAll() // 인증이 필요하지 않은 유일한 엔드포인트,리액트의 url과 일치시켜야 함
